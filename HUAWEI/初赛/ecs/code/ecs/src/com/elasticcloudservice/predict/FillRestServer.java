...
public class FillRestServer {
    ...
    int serverCPU = 0; int serverMEM = 0;
    int restCPU = 0; int restMEM = 0;
    ...
    public int[][] Fill(int[][] server, int sever_cpu, int server_mem, int sever_count,
        ...
        if (Predict.type == "CPU") {
            Collections.sort(vector, comparator);//根据cpu排序从大到小
        }
        else{
            Collections.sort(vector,comparator1);//根据mem排序从大到小
        }

        //遍历服务器计算剩余cpu和mem
        String name = ""; int num = 0; String[] temp; int v = 0; double tmp = 0;
        for (int i = 1; i < sever_count; i++){
            restCPU = sever_cpu; restMEM = server_mem;
            for (int j = 0; j < flavor_name.length; j++){
                if (map.get(flavor_name[j]) != null) {
                    restCPU = restCPU - server[i][j+1] * map.get(flavor_name[j]).cpu;
                    restMEM = restMEM - server[i][j+1] * map.get(flavor_name[j]).mem;
                }
            }
        ...
        //处理最后一个服务器
        restCPU = serverCPU; restMEM = serverMEM;
        for (int i = 0; i < flavor_name.length; i++) {
            if (map.get(flavor_name[i]) != null) {
                restCPU = restCPU - server[sever_count][i + 1] * map.get(flavor_name[i]).cpu;
                restMEM = restMEM - server[sever_count][i + 1] * map.get(flavor_name[i]).mem;
                Predict.flavorsinlastserver.put(flavor_name[i], server[sever_count][i+1]); // 最后一个服务器内的虚拟机
            }
        }
        cpu_usage_rate = (double) restCPU / serverCPU;
        mem_usage_rate = (double) restMEM / serverMEM;
        if (Predict.type =="CPU")
            tmp = cpu_usage_rate;
        else
            tmp = mem_usage_rate;
        if (tmp > 0.68){
            Predict.server_count -= 1;
        }
        else {
            Predict.flavorsinlastserver.clear();
            for (int k = 0; k < vector.size(); k++) {
                if (restCPU >= vector.get(k).cpu && restMEM >= vector.get(k).mem){
                    num = (restCPU/vector.get(k).cpu) < (restMEM/vector.get(k).mem)
                            ? restCPU/vector.get(k).cpu : restMEM/vector.get(k).mem; //填补的数量 待优化
